...
braucht sie trotzdem — dafür dieses Skript, das nur auf dem Server läuft:
    ...
    .venv/bin/python scripts/qa_verlauf.py person@example.org
    ...
    try:
        ...
        for g in gespraeche:
            ...
            for t in turns:
                ...
                print(f"\n  [{str(t['created'])[:16]}] FRAGE: {t['question']}")
                ...
                if not args.kurz:
                    for row in (t["answer"] or "").splitlines():
                        if row.strip():
                            print(f"    {row}")
                for s in quellen[:12]:
        ...
        for r in ratslotse._conn.execute(
                "SELECT question, status, length(report) n, created FROM deep_research_jobs "
                "WHERE user_id = ? ORDER BY created", (uid,)):
            print(f"  {str(r['created'])[:16]} {r['status']:12s} "
                  f"{r['n'] or 0:5d} Zeichen · {r['question']}")

        print("\n=== Feedback ===")
        for f in council._conn.execute(
                "SELECT created, rating, question, reason FROM council_qa_feedback "
                "WHERE user_id = ? ORDER BY created", (uid,)):
            print(f"  {str(f['created'])[:16]} {f['rating']} · {f['question'][:70]}"
                  + (f"\n      Grund: {f['reason']}" if f["reason"] else ""))
