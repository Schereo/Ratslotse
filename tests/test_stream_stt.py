...
def test_windower_dispatches_after_settle_and_flushes_on_close():
    seen = []
    w = stream_stt.Windower(lambda idx, segs, closing: seen.append((idx, segs, closing)), window_seconds=15, settle=6)
    w.add((3.0, "eins"))
    w.advance(15.0)
    assert seen == []            # Fenster 0 endet bei 15 s, wartet aber 6 s auf Nachzügler
    w.advance(21.0)
    assert seen == [(0, [(3.0, "eins")], False)]
    w.add((16.0, "zwei")); w.add((14.5, "nachzügler"))
    w.advance(50.0)              # Fenster 1 (15–30) fällig ab 36 s, Fenster 2 (30–45) erst ab 51 s
    assert [s[0] for s in seen] == [0, 1]
    assert seen[1][1] == [(16.0, "zwei"), (14.5, "nachzügler")]
    w.add((52.0, "schluss"))
    w.close()
    assert seen[-1] == (2, [(52.0, "schluss")], True)


def test_windower_survives_a_failing_tracker(caplog):
    def kaputt(idx, segs, closing):
        raise RuntimeError("Tracker tot")
    w = stream_stt.Windower(kaputt, window_seconds=15, settle=0)
    with caplog.at_level("ERROR"):
        w.advance(15.0)
    assert w.dispatched == 1 and "Fenster 0 fehlgeschlagen" in caplog.text
...
def test_record_and_transcribe_streams_windows_and_stops_at_closing(monkeypatch):
    ...
    segs = stream_stt.record_and_transcribe(
        on_window=lambda idx, s, closing: windows.append((idx, [t for _, t in s], closing)),
    ...
    # Fenster 0 (0–15 s) fällig ab 21 s Audio, Fenster 1 ab 36 s; dann Schluss
    # bei 52 s → Rest als letztes Fenster mit closing.
    assert windows[0] == (0, ["Wir kommen zu Punkt 6.1."], False)
    assert windows[1] == (1, ["Frau Drügemöller, bitte."], False)
    assert windows[-1][2] is True and "Damit schließe ich die Sitzung." in windows[-1][1]
...
def test_recorder_falls_back_to_chunks_when_streaming_is_unavailable(monkeypatch):
    ...
    tracker = SimpleNamespace(on_chunk=lambda *a: None, people=[], chunk_seconds=15)
