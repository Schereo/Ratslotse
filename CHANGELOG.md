...
### Geändert
- **Nutzer*innen im Admin-Bereich gezielt finden.** Die Kontenliste zeigt aktive
  Tage für Web und App, lässt sich nach letzter Nutzung oder Nutzungshäufigkeit
  sortieren und nach Inaktivität filtern. Gesperrte Konten lassen sich
  ausblenden. (#1394)
...
- **Die Kurzfassungen werden genauer — und ein Ausfall bleibt nicht mehr
  still.** Die Tragweite-Gründe und TOP-Kurzfassungen laufen jetzt auf
  `gpt-5.6-luna` (vorher DeepSeek v4 Pro bzw. zwei Jahre altes gpt-4o-mini) — im
  Vergleich an 55 echten Tagesordnungspunkten das einzige Modell, das Beträge,
  Flächen und Ortsnamen durchgängig behält, bei einem Viertel der Kosten. Alle
  drei betroffenen Modelle sind per `COUNCIL_*_MODEL` tauschbar. Der
  Tragweite-Prompt sagt dem Modell außerdem, dass Vereinfachung nie verschärfen
  darf: Aus einer Genehmigungspflicht wird kein „Verbot" mehr (Leserfeedback zur
