...
(`nwz-web-frontend`, :3000) auf dem App-Server `tk-nwz`. Nach außen muss jemand
TLS terminieren und auf den Next.js-Port proxen. Frühere Variante: lokales nginx
auf `tk-nwz`. Die Topologie hat aber bereits eine **Edge-VM** (`tk-edge-vm`) als
...
und `reverse_proxy`t auf `tk-nwz:3000`. Auf `tk-nwz` läuft **kein** nginx mehr;
