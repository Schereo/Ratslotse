        ...
        let raw = #"""
        {
          ...
          "evidence": {
            ...
            "parteien": [{
              ...
              "einig": true,
              "beitraege": 4,
              "kernaussage": {"text": "Wer den Bus schneller macht, entlastet die Straße für alle anderen.", "sprecher": "Mara Beispiel", "datum": "2026-08-26"}
            }, {
              "partei": "BSW",
              "haltung": "dagegen",
              ...
              "einig": false,
              "beitraege": 2,
              "kernaussage": {"text": "Acht Millionen für zwei Kilometer Busspur muss man den Leuten erst einmal erklären.", "sprecher": "Jonas Muster", "datum": "2026-08-26"}
            }],
            "debatten": [{
              ...
              "sprecher": "Jonas Muster",
              "partei": "BSW",
              "art": "Rede",
              "datum": "2026-08-26",
              "auszug": "Die Kosten je Kilometer Busspur seien im Vergleich zu anderen Städten zu hoch angesetzt."
            }, {
              "sprecher": "Herr Beispielhaft",
              "art": "Rede",
              "datum": "2026-08-26",
              "auszug": "Die Verwaltung verweist auf die Förderkulisse des Landes; ohne sie trüge die Stadt den vollen Betrag."
            }],
            "planungen": [{
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ForEach(Array(press.enumerated()), id: \.offset) { _, item in
                        let row = EvidenceTextRow(
                            ...
                            detail: item["auszug"]?.string,
                            meta: RatsDate.short(item["datum"]?.string),
                            symbol: .newspaper
                        )
                        if let raw = item["url"]?.string, let url = URL(string: raw) {
                            Link(destination: url) { row }.buttonStyle(RatsPlainButtonStyle())
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ForEach(Array(debates.enumerated()), id: \.offset) { _, item in
                        ...
                        let row = EvidenceTextRow(
                            ...
                            party: party,
                            detail: item["auszug"]?.string,
                            meta: [kind, RatsDate.short(item["datum"]?.string)].compactMap { $0 }.joined(separator: " · "),
