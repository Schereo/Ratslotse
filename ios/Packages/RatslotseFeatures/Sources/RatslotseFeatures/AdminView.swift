...
private struct AdminUsersView: View {
    ...
    var body: some View { adminList(title: "Nutzer*innen", subtitle: "\(users.count) Konten") {
        ...
        ForEach(filtered) { user in VStack(alignment: .leading, spacing: 10) { HStack { Text(user.email).font(RatsFont.body(14, weight: .semibold)).lineLimit(1); Spacer(); Text(user.role).font(RatsFont.mono(9)).foregroundStyle(user.role == "admin" ? RatsColor.signal : RatsColor.secondary) }; Text("\(user.nTopics) Themen · \(user.nAbos) Abos · \(user.nKi) KI-Fragen · \(user.nQuiz) Quiz").font(RatsFont.body(11)).foregroundStyle(RatsColor.secondary); HStack { if user.role == "moderator" { Text("Nur im Bürgerportal verwaltbar").font(RatsFont.body(12, weight: .semibold)).foregroundStyle(RatsColor.secondary) } else { Menu(roleLabel(user.role)) { Button("Nutzer*in") { Task { await update(user, key: "role", value: "user") } }; Button("Admin") { Task { await update(user, key: "role", value: "admin") } } }.buttonStyle(.bordered); Button(user.status == "active" ? "Sperren" : "Freischalten") { Task { await update(user, key: "status", value: user.status == "active" ? "pending" : "active") } }.buttonStyle(.bordered).disabled(user.id == model.user?.id) } } }.ratsCard() }
        if let error { ErrorCard(message: error) { Task { await load() } } }
    }.task { await load() } }
    private func roleLabel(_ role: String) -> String { role == "admin" ? "Admin" : role == "moderator" ? "Moderation" : "Nutzer*in" }
