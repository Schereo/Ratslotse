...
| `<MeldeRanglisteGrafik>` | `melde-rangbalken.tsx` | Tiefe Bürgerportal-Rangliste: `zeilen {key, label, wert, haeufigkeit, offen, umschalten, vorschau} · beleg`. Das Modul besitzt Rang/Maximum, eine gemeinsame proportionale Null-bis-Max-Schiene, Aufklapp-Semantik, Motion und responsive Hierarchie; die Seite liefert nur öffentliche Zeilendaten, Zustand und fachlichen Vorschauinhalt. Alle Ränge stehen auf Desktop und Mobil in gleich breiten Zeilen untereinander, damit 18 = volle Breite, 11 ≈ 61 % und 7 ≈ 39 % unmittelbar vergleichbar bleiben. Die ersten drei erhalten nur eine kompakte Tönungs-, Schrift- und Ranghierarchie. Die Fülllänge hat bewusst keine Mindestbreite; die exakte Zahl steht daneben. Vorschauen fließen unter ihrem Toggle im selben Zeilenkasten. Eintritt und Balken dauern höchstens 300 ms, `prefers-reduced-motion` zeigt sofort den Endwert. Der Beleg steht einmal sichtbar direkt unter allen Balken. Einsatz: Tab „Rangliste“ (`view=meistgemeldet` bleibt kompatibel). |
...
- Haushaltsgrafiken verwenden nur die Rampen-Tokens `--hh-ein-*` /
  ...
  schreiben keine Sonderfarben. Die interne Melderangbalken-Implementierung
  nimmt ebenfalls keine Farb-Prop: Sie verwendet dieselben zentralen
...
- Animation: nur CSS-Transitions/-Keyframes, ≤300 ms,
