...
| `<Treemap>` (GB-08) | `treemap.tsx` | `knoten {key, name, value, gruppe, zusatz?}[] · farbe(gruppe) · textFarbe?(gruppe) · buendelnAb · treffer? · aufRest? · nomen? · flaecheLabel? · verworfenSatz? · anteil?`. Fläche ∝ Gesamtsumme (Geometrie in `kachelflaeche.ts`, zur Laufzeit), Rest-Kachel ist Pflicht, **wo gebündelt wird** (neutral schraffiert — gebündelt ist keine Lücke); zerlegt die Fläche eine geschlossene Liste, rechnet die Seite den Schnitt mit `buendelGrenze()` und gibt `restZusatz` (die Aufzählung der gebündelten Posten — steht in Ablesekarte, Legende und Mobil-Zeile) sowie `anteil` mit, dann nennen Kacheln und Ablesekarte den Prozentwert. **Die Schrift folgt der Fläche** (`textstufe`, seit 02.09.): groß = Bricolage 28 px mit Einheit und Anteil, mittel 18 px, klein die 11-px-Zeile; die Einheit steht auf jeder Kachel mit Platz (`traegtEinheit`). **Die Rest-Kachel klappt auf** (ohne `aufRest`, bis 40 Posten): Sie wächst zur Mappe über die ganze Fläche, die gebündelten Posten liegen als Kacheln darin, das Kopfband trägt Rückweg und Summe, jeder Anteil misst weiter am Ganzen; Escape schließt. Mit `aufRest` (Investitionen: Tausende) führt sie stattdessen in die Suche. **Bewegung:** einmalige Staffel beim Aufbau, Lage/Maß als Übergang beim Jahrgangswechsel, Spotlight unter dem Zeiger (`.kf-*` in `globals.css`, reduced-motion still). Die **Ablesekarte** unter dem Bild (Farbmarke, Name, Zusatz, Summe, Anteilsbalken) zeigt die aktive Kachel und ohne eine die Gesamtsumme — nie eine Leerstelle. `textFarbe` ist Pflicht auf einer Karte: `--hh-seg-text` trägt nur das laute Ende der Rampe, den Rest sagt `rampenText()`. Die Wörter (`nomen`, `flaecheLabel`, `verworfenSatz`) kommen von der Seite — die Vorgaben sind der Investitionen-Fall. Nur positive Werte; Verworfenes steht als Satz. Unter 520 px Containerbreite rendert sie selbst eine `<RanglisteSchiene>` — gleiche Daten, gleiche Sortierung (H4-A). Einsatz: Investitionen-Explorer, Herkunftsseite des Flussbilds. |
...
- **`namenszeilen(breite, hoehe)`** — wie viele Zeilen der Name bekommt.
  ...
  der Legende.
- **`textstufe(breite, hoehe)` / `namenszeilenStufe` / `traegtEinheit` /
  `TEXTMASSE`** — die drei Beschriftungsstufen und ihre Zeilenmaße. Die
  Komponente setzt die Zeilenhöhen aus `TEXTMASSE` als feste Pixel: Ein
  `button` erbt 16 px / 24 px, und ein Zahlenblock, der diese Strebe mitnimmt,
  ist 24 statt 16 px hoch — der Name darüber wurde gestaucht und
  „Abrissmaßnahmen" in der dritten Zeile durchgeschnitten (gemessen 02.09.).
  Die Probe prüft die Treppe (größere Kachel nie kleiner gesetzt), die
  Gleichheit mit `namenszeilen()` auf der kleinen Stufe und dass der größte
  Ertragsposten bei jeder Breite groß steht.
