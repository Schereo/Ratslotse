 ...
 *
 * Die Karte hat von Anfang an ihre End-Höhe: ein unsichtbarer „Sizer" rendert
 * den fertigen Zustand und reserviert den Platz, der Live-Inhalt liegt absolut
 * darüber. So wächst die Seite während des Streamens nicht (kein Layout-Shift),
 * und das bleibt ohne Pixel-Magie über alle Viewports korrekt.
 */
...
/** Der Karten-Inhalt für einen gegebenen Zustand — einmal unsichtbar als Sizer
 *  (phase="done"), einmal live animiert. Gleiche Struktur = gleiche Höhe. */
...
export function LandingQaDemo() {
  ...
  return (
    <div ref={ref} className="relative mt-8 rounded-2xl border border-border bg-card shadow-lifted">
      {/* Sitzt auf der Kartenkante (halb über dem Rahmen) — dort kann kein
          Inhalt (z. B. der „Fragen"-Button) das Badge verdecken. */}
      <span className="absolute -top-2.5 right-5 z-10 rounded-full border border-border bg-muted px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground shadow-sm">
        Demo
      </span>

      {/* Sizer: der fertige Zustand, unsichtbar — hält die Karte auf End-Höhe. */}
      <div className="invisible p-5 sm:p-6" aria-hidden>
        <DemoContent phase="done" typed={QUESTION} streamed={ANSWER} />
      </div>

      {/* Live-Inhalt, deckungsgleich über dem Sizer. */}
      <div className="absolute inset-0 p-5 sm:p-6">
        <DemoContent phase={phase} typed={typed} streamed={streamed} onReplay={replay} />
      </div>
    </div>
  );
}
