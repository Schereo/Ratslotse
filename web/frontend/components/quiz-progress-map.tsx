...
const LEGEND_FALLBACK = ["unentdeckt", "entdeckt", "vertraut", "gemeistert"];
...
export function QuizProgressMap({ districts: mine, districtsAll, legend, onPlay, className }: {
  districts: NonNullable<QuizStats["districts"]>;
  /** Dieselbe Karte über alle Mitspielenden (Plan Q11) — Umschalter „Du / Alle". */
  districtsAll?: QuizStats["districts_all"];
  legend?: QuizStats["district_legend"];
  onPlay: (district: string) => void;
  className?: string;
}) {
  const [view, setView] = useState<"mine" | "all">("mine");
  const districts = view === "all" && districtsAll ? districtsAll : mine;
  const LEGEND = legend?.[view] ?? LEGEND_FALLBACK;
    ...
    <section className={cn("grid gap-3 rounded-2xl border border-border bg-card p-4 sm:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] sm:grid-rows-[auto_1fr] sm:gap-x-5", className)}>
      <div className="sm:col-start-2 sm:row-start-1 sm:mt-1">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-foreground">{view === "mine" ? "Deine Stadtkarte" : "Wie gut kennt Oldenburg …"}</h2>
          {districtsAll && (
            <div role="tablist" aria-label="Ansicht der Karte" className="inline-flex rounded-lg border border-border p-0.5 text-xs">
              {(["mine", "all"] as const).map((v) => (
                <button key={v} type="button" role="tab" aria-selected={view === v} onClick={() => setView(v)}
                  className={cn("rounded-md px-2.5 py-1 font-medium transition-colors duration-tipp",
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {view === "mine" ? (
            ...
            <>Die Trefferquote aller, die mitspielen, je Ortsbereich — erst ab 20 Antworten, damit sie etwas sagt.</>
          )}
        </p>
      </div>
