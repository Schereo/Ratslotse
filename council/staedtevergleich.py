...
def lies_kfa(pfad: str) -> KfaJahrgang:
    ...
    years = [(i, int(m.group(1))) for i, t in sorted(spalten.items())
             if (m := re.search(r"Steuerkraftmesszahlen\s+(\d{4})", t))]
    if c_key is None or c_name is None or c_ew is None or len(years) < 2:
        raise ValueError(
