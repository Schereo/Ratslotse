...
class CouncilStore:
    ...
    # --- related entities (precomputed, council.related) ---------------------
    def entity_rows(self) -> list[dict]:
        """All entities (id/slug/name/kind/n) — input for the relation backfill."""
        return [dict(r) for r in self._conn.execute(
            "SELECT id, slug, name, kind, n FROM council_entities")]

    def entity_link_rows(self) -> list[tuple]:
        """All (entity_id, decision_id) links — input for the relation backfill."""
